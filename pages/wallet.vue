  ...
  <div class="bg-gray-50">
    ...
    >
      <div class="flex flex-col space-y-1">
        ...
        <h4 v-if="wallet.bech32.length" class="font-medium text-gray-600">
          {{ wallet.bech32 }}
        </h4>
        <div v-else class="h-6 w-64 bg-gray-200 rounded-2xl mb-2" />
      </div>
      <div v-if="userDucks.length > 0" class="my-8">
        <h4 class="text-3xl font-semibold text-gray-800 mt-12">
          {{ userDucks.length }} ducks
        </h4>
        <div
          v-if="this.userDucks.length > 0"
          class="
            grid grid-cols-1
            md:grid-cols-2
            lg:grid-cols-3
      ...
      <div v-else class="my-8">
        <div class="h-72 w-full rounded-2xl bg-gray-200 flex items-center">
          <p
            ...
            "
          >
            Connect your wallet to see your ducks
          </p>
        </div>
      </div>
    </div>
...
export default {
  ...
  methods: {
    fetchUserDucks() {
      this.$store.dispatch(
        "explore/fetchUserDucks",
        this.$store.state.wallet.wallet.base16
      );
    },
  },
  async beforeMount() {
